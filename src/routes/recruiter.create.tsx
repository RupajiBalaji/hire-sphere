...
export const Route = createFileRoute("/recruiter/create")({
...
function CreateTask() {
  ...
  function addGuideline() {
    ...
    if (trimmed && !guidelines.includes(trimmed)) {
    ...
    <div className="pb-24">
      ...
      <div className="space-y-5 px-5 pt-5">
        ...
        <div className="rounded-3xl border border-border bg-card p-4">
          ...
          <div
            className="rounded-2xl p-4 text-white shadow-elevated transition-all"
          ...
          >
            ...
            <h3 className="mt-3 text-sm font-extrabold leading-snug">
              {title || "Super Awesome Task Title"}
            </h3>
        ...
        <div className="space-y-4">
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder="e.g. Design a high-conversion checkout page"
          ...
          <label className="block">
            ...
            <textarea
              ...
              placeholder="Detail the instructions, context, goals and scope of the task for candidates..."
              rows={4}
                ...
                {EMOJIS.map((e) => (
                  <button
                    ...
                    className={`flex h-9 w-9 items-center justify-center rounded-xl text-base transition-all ${
                      logo === e ? "bg-primary text-white scale-110 shadow-sm" : "bg-muted text-foreground hover:bg-muted/80"
                    }`}
                  >
                    {e}
                  </button>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {GRADIENTS.map((g) => (
                  <button
                    ...
                    className={`h-7 w-7 rounded-full transition-all ring-offset-2 ${
                      accent === g.value ? "ring-2 ring-primary scale-110" : "hover:scale-105"
                    }`}
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full rounded-xl border border-border bg-background px-2.5 py-2.5 text-xs text-foreground outline-none focus:border-primary"
              >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {skills.map((s) => (
                <span key={s} className="flex items-center gap-1 rounded-full bg-gradient-soft px-2.5 py-1 text-xs font-bold text-primary animate-fade-in">
                  {s}
                  <button type="button" onClick={() => setSkills(skills.filter((x) => x !== s))} className="ml-0.5 text-primary hover:text-destructive">
            ...
            <div className="flex gap-2">
              ...
              />
              <button type="button" onClick={addSkill} className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary text-white">
                <Plus className="h-3.5 w-3.5" />
              </button>
            </div>
          </div>

          {/* Submission Guidelines Editor */}
          <div>
            ...
            <ul className="space-y-2 mb-2">
              {guidelines.map((g, idx) => (
                <li key={idx} className="flex items-center justify-between rounded-xl bg-muted/40 p-2.5 text-xs text-foreground">
                  <span className="truncate flex-1">{g}</span>
                  <button type="button" onClick={() => setGuidelines(guidelines.filter((_, i) => i !== idx))} className="text-muted-foreground hover:text-destructive">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Add guideline (e.g. Figma file, Hosted URL)"
                className="flex-1 rounded-xl border border-border bg-background px-3 py-2 text-xs outline-none focus:border-primary"
              />
              <button type="button" onClick={addGuideline} className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary text-white">
                <Plus className="h-3.5 w-3.5" />
              </button>
            </div>
          </div>
      ...
      <div className="fixed bottom-[60px] left-1/2 z-50 w-full max-w-[440px] -translate-x-1/2 border-t border-border/60 bg-background/90 px-5 py-4 backdrop-blur-xl">
        ...
        >
          {createTaskMutation.isPending ? "Posting Task..." : (
            <>
