...
import {
  getRecruiterProfile, saveRecruiterProfile,
  type FullProfile,
} from "@/lib/api/profile";
import {
  User, MapPin, Briefcase, Building2,
  Link as LinkIcon, Plus, Trash2, Check, ChevronDown,
  Globe, Github, Linkedin, Twitter, Sparkles, Save,
} from "lucide-react";

export const Route = createFileRoute("/recruiter/edit-profile")({
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-sm">
      ...
      >
        ...
        {open ? <ChevronDown className="h-4 w-4 text-muted-foreground rotate-180 transition-transform" /> : <ChevronDown className="h-4 w-4 text-muted-foreground transition-transform" />}
      </button>
  ...
  return (
    <div className="pb-24">
      <ScreenHeader
        title="Edit Profile"
        back={
          <button type="button" onClick={() => navigate({ to: "/recruiter/profile" })}
      ...
      <div className="space-y-4 px-4 pt-4">
        ...
        <Section icon={<User className="h-4 w-4" />} title="Recruiter Identity">
          ...
          <div className="space-y-3">
            ...
            <Field label="Headline" value={form.headline} onChange={(v) => set("headline", v)} placeholder="e.g. Senior Technical Recruiter · Talent Acquisition Lead" />
        ...
        <Section icon={<Sparkles className="h-4 w-4" />} title="About You">
          <Field
            ...
            placeholder="Tell candidates about your role, what kind of talent you're looking for, and why people should work at your company."
            rows={5}
          />
