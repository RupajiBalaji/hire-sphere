  ...
  return (
    <div className="pb-24">
      <ScreenHeader
        title="Edit Profile"
        back={
          <button type="button" onClick={() => navigate({ to: form?.role === "recruiter" ? "/recruiter/profile" : "/app/profile" })}
      ...
      <div className="space-y-4 px-4 pt-4">
        ...
        <Section icon={<User className="h-4 w-4" />} title="Identity">
          <div className="mb-4 flex items-center gap-4">
            <div className="relative">
              ...
              {/* Camera overlay — LinkedIn-style add photo button */}
              <label
            ...
            <div className="flex-1">
              ...
              {form.avatar && (
                <button
                  type="button"
                  onClick={() => set("avatar", "")}
                  className="mt-1.5 text-[11px] font-semibold text-destructive hover:underline"
                >
                  Remove photo
                </button>
              )}
        ...
        <Section icon={<Sparkles className="h-4 w-4" />} title="About">
          <Field
            ...
            placeholder="Tell recruiters and collaborators about yourself — what you've built, what excites you, what you're looking for."
            rows={5}
          />
        ...
        <Section icon={<LinkIcon className="h-4 w-4" />} title="Links">
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <Field label="GitHub" value={form.github} onChange={(v) => set("github", v)} placeholder="github.com/username" />
            </div>
            <div className="flex items-center gap-3">
              <Linkedin className="h-4 w-4 shrink-0 text-muted-foreground" />
              <Field label="LinkedIn" value={form.linkedin} onChange={(v) => set("linkedin", v)} placeholder="linkedin.com/in/username" />
            </div>
            <div className="flex items-center gap-3">
              <Twitter className="h-4 w-4 shrink-0 text-muted-foreground" />
              <Field label="X / Twitter" value={form.twitter} onChange={(v) => set("twitter", v)} placeholder="x.com/username" />
            </div>
          </div>
        </Section>
...
function ExperienceEditor({ items, onChange }: { items: WorkExperience[]; onChange: (v: WorkExperience[]) => void }) {
  ...
  return (
    <div className="space-y-4">
      {items.map((item) => (
        <div key={item.id} className="rounded-2xl border border-border bg-background p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => remove(item.id)} className="text-destructive"><Trash2 className="h-4 w-4" /></button>
          </div>
          ...
          <div>
            <span className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Employment type</span>
            <div className="flex flex-wrap gap-2">
              {EMP_TYPES.map((t) => (
                <button key={t} type="button" onClick={() => update(item.id, { type: t })}
                  className={`rounded-full px-3 py-1 text-xs font-semibold transition-all ${item.type === t ? "bg-primary text-white" : "border border-border text-muted-foreground"}`}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <span className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Start</span>
              <div className="flex gap-1">
                <select value={item.startMonth} onChange={(e) => update(item.id, { startMonth: e.target.value })} className="flex-1 rounded-xl border border-border bg-background px-2 py-2 text-xs outline-none focus:border-primary">
                  {MONTHS.map((m) => <option key={m}>{m}</option>)}
                </select>
                <select value={item.startYear} onChange={(e) => update(item.id, { startYear: e.target.value })} className="flex-1 rounded-xl border border-border bg-background px-2 py-2 text-xs outline-none focus:border-primary">
                  {YEARS.map((y) => <option key={y}>{y}</option>)}
                </select>
              </div>
            </div>
            <div>
              <span className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">End</span>
              <div className="flex gap-1">
                <select value={item.endMonth} onChange={(e) => update(item.id, { endMonth: e.target.value })} disabled={item.current} className="flex-1 rounded-xl border border-border bg-background px-2 py-2 text-xs outline-none focus:border-primary disabled:opacity-40">
                  {MONTHS.map((m) => <option key={m}>{m}</option>)}
                </select>
                <select value={item.endYear} onChange={(e) => update(item.id, { endYear: e.target.value })} disabled={item.current} className="flex-1 rounded-xl border border-border bg-background px-2 py-2 text-xs outline-none focus:border-primary disabled:opacity-40">
                  {YEARS.map((y) => <option key={y}>{y}</option>)}
                </select>
              </div>
            </div>
      ...
      <button type="button" onClick={add} className="flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-primary/40 py-3 text-sm font-semibold text-primary hover:border-primary hover:bg-primary/5">
...
function EducationEditor({ items, onChange }: { items: Education[]; onChange: (v: Education[]) => void }) {
  function add() {
    onChange([...items, { id: uid(), school: "", degree: "", field: "", startYear: "2021", endYear: "2025", grade: "", activities: "" }]);
  }
  ...
  return (
    <div className="space-y-4">
      {items.map((item) => (
        <div key={item.id} className="rounded-2xl border border-border bg-background p-4 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-primary">Education</span>
            <button type="button" onClick={() => remove(item.id)} className="text-destructive"><Trash2 className="h-4 w-4" /></button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <span className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Start year</span>
              <select value={item.startYear} onChange={(e) => update(item.id, { startYear: e.target.value })} className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary">
                {YEARS.map((y) => <option key={y}>{y}</option>)}
              </select>
            </div>
            <div>
              <span className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">End year</span>
              <select value={item.endYear} onChange={(e) => update(item.id, { endYear: e.target.value })} className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary">
                {YEARS.map((y) => <option key={y}>{y}</option>)}
              </select>
            </div>
          </div>
    ...
    <div className="space-y-4">
      {items.map((item) => (
        <div key={item.id} className="rounded-2xl border border-border bg-background p-4 space-y-3">
          ...
          <Field label="Description" value={item.description} onChange={(v) => update(item.id, { description: v })} placeholder="What problem does it solve? What did you build?" rows={3} />
          <Field label="URL / Link" value={item.url} onChange={(v) => update(item.id, { url: v })} placeholder="https://github.com/you/project" />
          <div>
            <span className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Tech tags</span>
            <div className="flex flex-wrap gap-2 mb-2">
              {item.tags.map((t) => (
                <span key={t} className="flex items-center gap-1 rounded-full bg-accent px-2.5 py-1 text-[11px] font-semibold text-foreground">
                  {t}
                  <button type="button" onClick={() => update(item.id, { tags: item.tags.filter((x) => x !== t) })}><Trash2 className="h-2.5 w-2.5" /></button>
                </span>
              ))}
            </div>
            <TagInput onAdd={(t) => update(item.id, { tags: [...item.tags, t] })} />
          </div>
        </div>
      ))}
      <button type="button" onClick={add} className="flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-primary/40 py-3 text-sm font-semibold text-primary hover:border-primary hover:bg-primary/5">
  ...
  return (
    <div className="flex gap-2">
      <input value={val} onChange={(e) => setVal(e.target.value)} onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), add())}
        placeholder="Add tag (e.g. React)" className="flex-1 rounded-xl border border-border bg-background px-3 py-2 text-xs outline-none focus:border-primary" />
      <button type="button" onClick={add} className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary text-white"><Plus className="h-3.5 w-3.5" /></button>
    </div>
  );
}
    ...
    <div className="space-y-4">
      {items.map((item) => (
        <div key={item.id} className="rounded-2xl border border-border bg-background p-4 space-y-3">
          ...
          <div>
            <span className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Issue date</span>
            <div className="flex gap-2">
              <select value={item.issueMonth} onChange={(e) => update(item.id, { issueMonth: e.target.value })} className="flex-1 rounded-xl border border-border bg-background px-2 py-2.5 text-sm outline-none focus:border-primary">
                {MONTHS.map((m) => <option key={m}>{m}</option>)}
              </select>
              <select value={item.issueYear} onChange={(e) => update(item.id, { issueYear: e.target.value })} className="flex-1 rounded-xl border border-border bg-background px-2 py-2.5 text-sm outline-none focus:border-primary">
                {YEARS.map((y) => <option key={y}>{y}</option>)}
              </select>
            </div>
          </div>
